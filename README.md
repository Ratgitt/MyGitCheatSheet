...
* `git push` - отправить изменения на удалённый репозиторий
   ...
   - (`origin --delete <название_ветки>` - удалить ветку в GitHub репозитории)
...
> **Ветка** (англ. _branch_) — это изолированный поток разработки проекта. Каждый член команды может работать в своей ветке и не мешать другим: коммиты, которые он сделает, не будут видны из других веток. А когда работа будет доделана, ветки можно соединить. 
...
> Название ветки - это указатель на последний коммит в этой ветке. То есть, хэш последнего коммита и название ветки - это одно и то же.

### Создание, навигация, сравнение

* `git branch <params>` - посмотреть, какие в проекте есть ветки и в какой из них вы сейчас находитесь.
   - (`<название-ветки>` - создать новую ветку)
   - (`-D <название_ветки>` - удалить ветку)
   - (`-d <название_ветки>` - безопасное удаление ветки. удаляет ветку, только если она была полностью объединена с другой — то есть если две ветки стали (или изначально были) частью одной истории)
* `git checkout <название_ветки>` - переключиться на другую ветку
   - (`-b <название_ветки>` - создать новую ветку и сразу перейти в неё)
* `git diff <ветка_a> <ветка_b>` - сравнить две ветки (последние коммиты в этих ветках)
   - (`~` - суффикс навигации. С его помощью можно сослаться на предыдущие коммиты. Например, если вы находитесь в ветке `main` и хотите вывести разницу между тем коммитом, который был три коммита назад, и текущим, нужно выполнить `git diff main~3 main`)
* `git merge <название_ветки>` - объединить ветки(слияние). Добавляет изменения из указанной ветки в текущую.
